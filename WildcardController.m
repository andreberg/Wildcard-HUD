...
- (void) awakeFromNib {
    ...
    [replaceNameTextField setFormatter:[[[ReplaceStringFormatter alloc] init] autorelease]];

    // 設定の復帰
    NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    id anObject = [defaults objectForKey:WildcardFindNameKey];
    if (anObject != nil) {
        [findNameTextField setStringValue:anObject];
    }

    anObject = [defaults objectForKey:WildcardReplaceNameKey];
    if (anObject != nil) {
        [replaceNameTextField setStringValue:anObject];
    }

    anObject = [defaults objectForKey:WildcardIgnoreCaseOptionKey];
    if (anObject != nil) {
        [ignoreCaseCheckBox setState:[anObject intValue]];
    }

    anObject = [defaults objectForKey:WildcardRegexOptionKey];
    if (anObject != nil) {
        [regexCheckBox setState:[anObject intValue]];
    }

    anObject = [defaults objectForKey:WildcardShowFinderKey];
    if (anObject != nil) {
        [showFinderCheckBox setState:[anObject intValue]];
    }

    anObject = [defaults objectForKey:WildcardDesktopKey];
    if (anObject != nil) {
        [desktopCheckBox setState:[anObject intValue]];
    }
...
// delimt by whitespace (simple matching syntax使用時)
{
    ...
    if (!_findInComment) {
        options |= OgreFindNotEmptyOption;                      // コメントを検索する場合以外は空マッチを許さない。
    }
    if ([ignoreCaseCheckBox state] == NSOnState) {
...
@"tell application \"Finder\"\n\
    ...
    *allItems = [NSMutableArray arrayWithCapacity:numberOfAllItems];
    for (i = 1 /* 注意! 0からではない */; i <= numberOfAllItems; i++) {
        [(NSMutableArray *)*allItems addObject:[[[results descriptorAtIndex:i] stringValue] precomposedStringWithCanonicalMapping]];
    }
#ifdef DEBUG_OGRE_WILDCARD
    ...
    if (comments) {
        ...
        *comments = [NSMutableArray arrayWithCapacity:numberOfAllItems];
        for (i = 1 /* 注意! 0からではない */; i <= numberOfAllItems; i++) {
            [(NSMutableArray *)*comments addObject:[[[results descriptorAtIndex:i] stringValue] precomposedStringWithCanonicalMapping]];
        }
        #ifdef DEBUG_OGRE_WILDCARD
...
- (OGRegularExpression *) regex {
    ...
    NS_DURING
    regex = [OGRegularExpression regularExpressionWithString:[[findNameTextField stringValue] precomposedStringWithCanonicalMapping]
...
@synthesize wildcardWindow;
@synthesize findNameTextField;
@synthesize ignoreCaseCheckBox;
@synthesize regexCheckBox;
@synthesize replaceNameTextField;
@synthesize showFinderCheckBox;
@synthesize statusTextField;
@synthesize findPopUp;
@synthesize replaceTitle;
@synthesize desktopCheckBox;
@synthesize renameButton;
@synthesize _findInComment;
@synthesize _targetWindowID;
@end
